...
def activate():
    try:
        ...
        if request.method == "GET": # LA ENCONTRÉ DE LA LINEA 18 (GET => OPTENER; POST => ENVIAR)
            number = request.args['auth'] 
            
            db = get_db() # LA ENCONTRÉ EN LA LINEA 14 (LLAMA A LA DB)
            attempt = db.execute(
                'SELECT id FROM activationlink WHERE challenge = ? AND  state= ? AND CURRENTE_TIMES BETWEEN created AND validation', (number, utils.U_UNCONFIRMED) # QUERY
            ).fetchone()
            ...
            if attempt is not None:
                db.execute(
                  ...
                  'UPDATE activationlink SET state = ? where id = ?',
                   (utils.U_CONFIRMED, attempt['id'])  # QUERY
                )
                db.execute(
                    'INSERT INTO user (username,password,salt,email) VALUES (?,?,?,?)',
                    (attempt['username'], attempt['password'], attempt['salt'], attempt['email']) # QUERY
                )
                db.commit()  # SI AY UN => DB.COMMIT() es porque se ha realizado un INSERT INTO  en la base de datos.
...
@bp.route('/register', methods=('GET', 'POST')) # FALTABA COMPLETAR CON LOS METODOS 'GET' & 'POST'
def register():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
      
        if request.method == "POST": # SE ENVIA LA INFO DEL FORMULARIO A LA DB   
            username = request.form["username"]  #solucion de error
            password = request.form["password"] #solucion de error
            email = request.form["email"]       #solucion de error
            
            db = get_db()                       #solucion de error
            error = None

            if not username:
                error = 'Username is required.'
                flash(error)
                return render_template('auth/register.html') # REDIRECCIONA AL REGISTRO DENTRO DE LA FUNCION => register
...
def confirm():
    try:
        ...
        if request.method == "POST": 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id FROM user WHERE  number = ?', (authid, utils.F_ACTIVE)).fetchone()
            
            if attempt is not None:
                db.execute(
                    'UPDATE forgotlink SET state = ? where id = ?', (utils.F_INACTIVE, attempt['id']) # query 
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                     'INSERT INTO forgotlink (password, salt, userid)  VALUE (?,?,?)' , # query                  
        ...
        return render_template('auth/forgo.html')
    except:
        return render_template('auth/forgot.html')
...
def change():
    try:
        ...
        if request.method == "POST": 
            ...
            db = get_db()  # ERROR CORREGIDO
            attempt = db.execute(
                'SELECT * FROM user WHERE  number = ?', (number, utils.F_ACTIVE)
            ).fetchone()
...
def login():
    try:
        ...
        if request.method == "POST":
            ...
            if error is None:
                session.clear()
                session['user_id'] = user["user_id"]  # modifique sin saber el USER
                return redirect(url_for('inbox.show'))
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.user = get_db().execute(
            'SELECT * FROM user WHERE user_id = ?', (user_id,)  # lo edité
        ).fetchone()

        
@bp.route('/logout')
def logout():
    session.get()    # modifique sin saber el .GET
    return redirect(url_for('auth.login'))
